...
- **Section headings** (`.team-section__title`): 40px / weight 600 / `-0.02em` / line-height 1.15 / `--c-dark` — copied directly from `about`'s repeated colored-block chapter titles (`.partners__title` / `.related-centers__title`), not `home`'s smaller 24px "What's New" list-header size. No border-bottom, matching `about`'s version of this heading. `.team-section--gradient` (the first section, "Core Leadership & Team") and `.team-section--purple` (Faculty Advisors) override the section background to the shared `--c-gradient` / `--c-accent` respectively and the heading to white, using `padding-block: var(--space-1000)` like `about`'s Orbital/Partners/Related-Centers color-block sections — Knight Fellows and Part Time Staff stay plain white.
